...
export function Header() {
  return (
    <header className="absolute left-1/2 top-0 z-50 w-full max-w-7xl -translate-x-1/2 flex items-center justify-between border-b border-black/10 py-4 px-4 sm:px-6 lg:px-8">
      <Button asChild className="h-auto bg-[#29261f] hover:bg-[#29261f]/90 px-4 py-1.5 text-sm">
        <Link href="#contact" className="font-body tracking-wide">
          CONTACT
        </Link>
      </Button>
      ...
      <Button variant="ghost" size="icon" className="w-9 h-auto p-0 md:hidden">
        <Image src="/hamburger.svg" alt="Menu" width={32} height={32} />
      </Button>
       <div className="hidden md:flex">
         <Button variant="ghost" size="icon" className="w-9 h-auto p-0">
            <Image src="/hamburger.svg" alt="Menu" width={32} height={32} />
          </Button>
       </div>
    </header>
